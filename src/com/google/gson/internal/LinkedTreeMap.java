...
public final class LinkedTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
	...
	Node<K, V> find(K key, boolean create) {
		...
		if (nearest != null) {
			...
			while (true) {
				...
				if (comparison == 0) { return nearest; }
		...
		if (nearest == null) {
			...
			if (comparator == NATURAL_ORDER && ! (key instanceof Comparable)) { throw new ClassCastException(
	...
	private abstract class LinkedTreeMapIterator<T> implements Iterator<T> {
		...
		final Node<K, V> nextNode() {
			...
			if (e == LinkedTreeMap.this.header) { throw new NoSuchElementException(); }
			if (LinkedTreeMap.this.modCount != this.expectedModCount) { throw new ConcurrentModificationException(); }
			this.next = e.next;
		...
		public final void remove() {
			...
			removeInternal(this.lastReturned, true);
